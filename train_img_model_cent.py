...
# Datasets
parser.add_argument('--root', type=str, default='F:\chrome downloads\dataset\VeRi', help="root path to data directory")
...
# Architecture
parser.add_argument('-a', '--arch', type=str, default='ainet', choices=models.get_names())
# Miscs
...
parser.add_argument('--start-eval', type=int, default=20, help="start to evaluate after specific epoch")
...
def main():
    ...
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss='cent', pretrained=False)
    print("Model size: {:.5f}M".format(sum(p.numel() for p in model.parameters())/1000000.0))

    criterion_xent = CrossEntropyLabelSmooth(num_classes=dataset.num_train_pids, use_gpu=use_gpu)
    criterion_cent = CenterLoss(num_classes=dataset.num_train_pids, feat_dim=model.feature_dim, use_gpu=use_gpu)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if (epoch+1) > args.start_eval and args.eval_step > 0 and (epoch+1) % args.eval_step == 0 or (epoch+1) == args.max_epoch:
            ...
            save_checkpoint({
                ...
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep' + str(epoch+1) + '.pth'))
